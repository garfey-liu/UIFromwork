    ...
    public class MemoryDetector
    {
        ...
        void OnGUI()
        {
            ...
            GUI.Label(this.allocMemoryRect,
                string.Format(TotalAllocMemroyFormation, UnityEngine.Profiling.Profiler.GetTotalAllocatedMemory() * ByteToM), bb);
            GUI.Label(this.reservedMemoryRect,
                string.Format(TotalReservedMemoryFormation, UnityEngine.Profiling.Profiler.GetTotalReservedMemory() * ByteToM), bb);
            GUI.Label(this.unusedReservedMemoryRect,
                string.Format(TotalUnusedReservedMemoryFormation, UnityEngine.Profiling.Profiler.GetTotalUnusedReservedMemory() * ByteToM), bb);
            GUI.Label(this.monoHeapRect,
                string.Format(MonoHeapFormation, UnityEngine.Profiling.Profiler.GetMonoHeapSize() * ByteToM), bb);
            GUI.Label(this.monoUsedRect,
                string.Format(MonoUsedFormation, UnityEngine.Profiling.Profiler.GetMonoUsedSize() * ByteToM), bb);
        }
    }
